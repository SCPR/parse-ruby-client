...
module Farse
  ...
  class Query
    ...
    def eq(field, value)
      add_constraint field, Farse.pointerize_value(value)
      self
    end

    def not_eq(field, value)
      add_constraint field, { "$ne" => Farse.pointerize_value(value) }
      self
    end
    ...
    def less_than(field, value)
      add_constraint field, { "$lt" => Farse.pointerize_value(value) }
      self
    end

    def less_eq(field, value)
      add_constraint field, { "$lte" => Farse.pointerize_value(value) }
      self
    end

    def greater_than(field, value)
      add_constraint field, { "$gt" => Farse.pointerize_value(value) }
      self
    end

    def greater_eq(field, value)
      add_constraint field, { "$gte" => Farse.pointerize_value(value) }
      self
    end

    def value_in(field, values)
      add_constraint field, { "$in" => values.map { |v| Farse.pointerize_value(v) } }
      self
    end

    def value_not_in(field, values)
      add_constraint field, { "$nin" => values.map { |v| Farse.pointerize_value(v) } }
      self
    end

    def related_to(field,value)
      h = {"object" => Farse.pointerize_value(value), "key" => field}
    ...
    def in_query(field, query=nil)
      query_hash = {Farse::Protocol::KEY_CLASS_NAME => query.class_name, "where" => query.where}
    ...
    def get
      ...
      if @class_name == Farse::Protocol::CLASS_USER
        uri = Protocol.user_uri
      elsif @class_name == Farse::Protocol::CLASS_INSTALLATION
        uri = Protocol.installation_uri
      end
      query = { "where" => where_as_json.to_json }
      set_order(query)
      [:count, :limit, :skip, :include].each {|a| merge_attribute(a, query)}
      Farse.client.logger.info{"Parse query for #{uri} #{query.inspect}"}
      response = Farse.client.request uri, :get, nil, query

      if response.is_a?(Hash) && response.has_key?(Protocol::KEY_RESULTS) && response[Protocol::KEY_RESULTS].is_a?(Array)
        parsed_results = response[Protocol::KEY_RESULTS].map{|o| Farse.parse_json(class_name, o)}
