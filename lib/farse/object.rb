...
module Farse
  ...
  class Object < Hash
    ...
    def eql?(other)
      Farse.object_pointer_equality?(self, other)
    end

    alias == eql?

    def hash
      Farse.object_pointer_hash(self)
    end
    ...
    def pointer
      Farse::Pointer.new(rest_api_hash) unless new?
    end
    ...
    def save
      ...
      data = Farse.client.request(self.uri, method, body)

      if data
        # array operations can return mutated view of array which needs to be parsed
        parse Farse.parse_json(class_name, data)
      end
    ...
    def safe_hash
      Hash[self.map do |key, value|
        if Protocol::RESERVED_KEYS.include?(key)
          ...
          [key, Farse.pointerize_value(value)]
        end
      end.compact]
    end
    ...
    def rest_api_hash
      self.merge(Farse::Protocol::KEY_CLASS_NAME => class_name)
    end
    ...
    def refresh
      if @parse_object_id
        data = Farse.get @class_name, @parse_object_id
        clear
        if data
          parse data
        end
    ...
    def parse_delete
      if @parse_object_id
        response = Farse.client.delete self.uri
      end
    ...
    def increment(field, amount = 1)
      ...
      body = {field => Farse::Increment.new(amount)}.to_json
      data = Farse.client.request(self.uri, :put, body)
      parse data
      self
    end
    ...
    def array_op(field, operation, value)
      ...
      if @parse_object_id
        ...
        @op_fields[field].objects << Farse.pointerize_value(value)
      end
